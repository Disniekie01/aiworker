...
class VRTrajectoryPublisher(Node):
    ...
    def __init__(self):
        ...
        insecure_vr = os.environ.get('ROBOTIS_VUER_INSECURE', '').strip().lower() in (
            '1', 'true', 'yes', 'on'
        )
        ...
        self.vuer = Vuer(
            host='0.0.0.0',
            port=8012,
            cert=None if insecure_vr else cert_file,
            key=None if insecure_vr else key_file,
            ws=ws_url,
        ...
        scheme = 'http' if insecure_vr else 'https'
        self.get_logger().info(
            f'VR Trajectory server available at: {scheme}://{hostname}:8012 '
            f'(insecure={insecure_vr})'
        )
        ...
        self.disable_squeeze_gate = os.environ.get(
            'ROBOTIS_VUER_DISABLE_SQUEEZE_GATE', ''
